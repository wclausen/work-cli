...
class RealGitService(private val gitClient: Git) : GitService {
    ...
    override fun checkout(branchName: String): Result<Ref, GitService.GitError> = com.github.michaelbull.result.runCatching {
        val branchAlreadyExists = gitClient.branchList()
            .call()
            .any { it.name.contains(branchName) }
        gitClient.checkout()
            .setName(branchName)
